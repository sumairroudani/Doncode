...
.controller('AppCtrl', function($scope, $timeout, $window, $ionicSlideBoxDelegate, dataFactory) {
	...
	$scope.asyncCount = function(){
		...
		dataFactory.service('POST','http://testapp.octantapp.com:8282/api/message_count',{donor_id:App_Session.donor_id}).
		then(function(res){
...
.controller('FeedsController', function($scope, dataFactory) {
	...
	dataFactory.service("POST","http://testapp.octantapp.com:8282/api/msg_feeds",{'donor_id':donid}).
	...
	$scope.isreadchk = function(message_id,link){
		...
		if(message_id!=false){
			dataFactory.service('POST','http://testapp.octantapp.com:8282/api/message_read/123456789',{'msg_id':message_id, 'donor_id':donid}).
				success(function(data, textStatus, xhr) {
					console.log(data);
				}).
...
.controller('FeedController', function($scope, $stateParams, dataFactory) {
	...
	if(AllFeeds[index]||AllFeeds[index]!=undefined){
		...
		dataFactory.service('POST','http://testapp.octantapp.com:8282/api/message_read/123456789',{'msg_id':msgid, 'donor_id':donid}).
			success(function(data, textStatus, xhr) {
				console.log(data);
			}).
			error(function(data, textStatus, xhr) {
...
.controller('ProfileController', function($scope,dataFactory,$ionicPopup,md5,$timeout) {
	...
	$scope.updateUser = function(){
		...
		dataFactory.service('PUT','http://testapp.octantapp.com:8282/api/donor',$scope.profile).
...
.controller('orgController', function($scope, dataFactory,$stateParams,$ionicModal) {
	...
	dataFactory.service('GET','http://testapp.octantapp.com:8282/api/organization').
	then(function(res){
		ob = res.data.feed_id;
		co = {}
		for(key in ob){
		...
		dataFactory.service('POST','http://testapp.octantapp.com:8282/api/donor_org_get',{donor_id:App_Session.donor_id,is_active:1}).
		then(function(res){
			console.log(res.data);
			for(key in res.data){
	...
	$scope.conferOrgs = function(){
		...
		if(clength>0){
			...
			dataFactory.service('POST','http://testapp.octantapp.com:8282/api/donor_org',$scope.data).
				then(function(res){
					r = res;
					console.log(res);
...
.controller('forgotController', function($scope, dataFactory, $ionicPopup) {
		...
		if(qCheck){
			if($scope.forgot.sec_answer.length>6){
	    		...
	    		if(re.test($scope.forgot.email)){
	    			...
	    			dataFactory.service('POST','http://testapp.octantapp.com:8282/api/reset_pswrd',$scope.forgot).
	    				success(function(data, textStatus, xhr){
	    					console.log(data);
	    					if(!data.Error){
		    					$scope.getPass().then(function(d){
		    						...
		    						dataFactory.service('PUT','http://testapp.octantapp.com:8282/api/reset_pswrd_upd',$scope.data).
		    						then(function(res){
		    							// console.log(res);
	...
	$scope.EmailCreds = function(){
		...
		dataFactory.service('POST','http://testapp.octantapp.com:8282/api/forgetpassword/123456789',$scope.forgot).
		then(function(res){
			console.log(res.data);
		}).
		finally(function(){
...
.controller('tncController', function($scope, dataFactory) {
	...
	dataFactory.service('GET',"http://testapp.octantapp.com:8282/api/do/oct5678093672").
		then(function(res){
			k = res.data.tc_id;
			terms = [];
			// console.log(k);
...
.controller('privacyController', function($scope, dataFactory) {
	...
	dataFactory.service('GET',"http://testapp.octantapp.com:8282/api/pr/oct5678093672").
		then(function(res){
			k = res.data.tc_id;
			privacy = [];
			console.log(k);
...
.controller('LoginController', function($scope, facebookService, md5, dataFactory, $cordovaGeolocation,$timeout,$ionicPopup) {
  ...
  if(!rem){
	...
	$scope.loadm();
		...
		// console.log($scope.user);
		dataFactory.service('POST',"http://testapp.octantapp.com:8282/api/userlogin/123456789",
			$scope.user).
			then(function(res){
				...
				if(uid != "false"){
					...
					if(login_info==2){
						$scope.getPass().
						then(function(dat){
    						dataFactory.service('PUT','http://testapp.octantapp.com:8282/api/reset_pswrd_upd',{'password':dat,donor_id:App_Session.donor_id}).
    						then(function(res){
    							console.log(res);
    							dataFactory._alert('Updated Successfully');
    						})
...
.controller('SignupController', function($scope, $http, $state, dataFactory) {
	...
	$scope.signup = function(ev) {
		...
		dataFactory.service('Post', ' http://testapp.octantapp.com:8282/api/donor', $scope.newuser).
			success(function (data, status, headers, config) {
				console.log(data);
				console.log('success');
				if(data.Error && data.Message.code == "ER_DUP_ENTRY"){
...
.controller('DonateController', function($scope,$window,$ionicSlideBoxDelegate,$ionicPopup,$stateParams,dataFactory,$ionicModal,$timeout) {
		...
		if($scope.data.slide>0){
			...
			if($scope.data.amount>=min){
				if(ext_flag){
					...
					dataFactory.service('POST','http://testapp.octantapp.com:8282/scrape',$scope.d).
					then().
					finally(function(){
	...
	$scope.slideC = function(index){
		...
		if(index>0){
			...
			dataFactory.service('POST','http://testapp.octantapp.com:8282/api/defaultdon',{'org_id':$scope.billing.org_id}).
			then(function(res){
				console.log(res.data);
...
.controller('StripeController', function($scope,dataFactory) {
	...
	$scope.checkout = function(){
    	...
    	dataFactory.service('POST','http://testapp.octantapp.com:8282/scrape',$scope.postData).
    	then(function(res){
    		if(res.data.success){
    			dataFactory._alert(
		...
		if($scope.data.slide>0){
			...
			if($scope.data.amount>=minam){
				if(		address_line1 &&
					...
					dataFactory.service('POST','http://testapp.octantapp.com:8282/api/pledge',$scope.data).
					then(function(res){
						console.log(res);
						$scope.showAlert();
					})
...
.factory('dataFactory', function($http, $rootScope, $ionicPopup, $ionicLoading, $state, $cordovaGeolocation) {
	...
	return {
		...
		sec_question: function(){
		    return this.service('GET','http://testapp.octantapp.com:8282/api/sec_quest')
		}
	}
